...
| history.append-only.008 | chat-api/thread-context.ts | The models added on 2026-09-23 (gpt-6-sol, gpt-6-luna, claude-opus-5-5) only ever append: across four turns with tool rows each turn n is an exact prefix of turn n+1, and the document-hint placement follows the provider | integration | message/summary/document services mocked; `selectedModel` varied | Build the model input for consecutive turns per model | Exact prefix; `tail-message` for GPT-6, `developer-message` for Opus 5.5 |
...
| ID | Target file | Case title | Type | Preconditions/mocks | Steps | Expected outcome |
|---|---|---|---|---|---|---|
| chat-page.unit.provider-seam.cache.001 | provider-seam.ts | `promptCacheOptions` is sent for every GPT-6 and GPT-5.6 model | unit | `./provider` + `@ai-sdk/azure` stubbed (existing file mocks) | `resolveProvider` for gpt-6-sol / gpt-6-luna / gpt-5.6-sol / -terra / -luna | `providerOptions.openai.promptCacheOptions` equals `{ mode: "implicit", ttl: "30m" }`; `promptCacheKey` / `store` unchanged |
...
| chat-page.unit.provider-seam.anthropic-effort.001 | provider-seam.ts | Opus 5.5 gets adaptive thinking and its measured "xhigh"/"max" effort passed through | unit | existing file mocks | `resolveProvider` for claude-opus-5-5 with effort xhigh, then max | `thinking` is `{type:"adaptive"}`; `effort` equals the input; no `openai` block |
| chat-page.unit.provider-seam.anthropic-effort.002 | provider-seam.ts | A Claude model whose `supportedReasoningEfforts` does not name xhigh/max gets "high" (negative) | unit | same | `resolveProvider` for claude-opus-4-8 and claude-sonnet-5 with xhigh, then max | `effort` is "high" |
...
| chat-page.unit.models.default.001 | models.ts | `resolveDefaultModel` returns the code default when the env var is unset/blank | unit | logger mocked | Call with `undefined`, `""`, `"   "` | `CODE_DEFAULT_MODEL` (gpt-6-sol) each time |
...
| chat-page.unit.models.default.005 | models.ts | `DEFAULT_MODEL` equals the code default with no override present | unit | same | Read the export | gpt-6-sol, and it exists in MODEL_CONFIGS |
| chat-page.unit.models.default.006 | models.ts | Terra (the fallback default) runs at "medium" effort; the Sol models, including the default GPT-6 Sol, and gpt-5.5 at "low" | unit | same | Read `defaultReasoningEffort` for terra / gpt-5.6-sol / gpt-5.5 / gpt-6-sol | medium / low / low / low |
| chat-page.unit.models.default-fallback | models.ts | An undeployed code default falls back to terra: GPT-6 Sol when deployed; gpt-5.6-terra (with a logged error) when Sol has no deployment, also for an unknown or undeployed `DEFAULT_MODEL_ID`; a deployed `DEFAULT_MODEL_ID` still wins; GPT-6 Sol unchanged when nothing is deployed | unit | logger mocked; deployment names toggled on MODEL_CONFIGS | `resolveDefaultModel` with each deployment set | Values as listed |
| chat-page.unit.model-selection.fallback-deployed | chat-api/model-selection.ts + models.ts | GPT-6 Luna is the fallback model and the per-model limit fallback only goes to a deployed model: every configured `fallbackModel` is gpt-6-luna; a deployed gpt-6-luna is used; without it gpt-5.6-luna serves (via `resolveDeployedFallbackModel` / `FALLBACK_MODEL_CHAIN`); with neither the turn stays on the requested model; a model over its limit never falls back to itself | unit | usage/budget/downgrade services mocked; deployment names toggled on MODEL_CONFIGS | `resolveModelAndLimits` with `CheckLimits` exceeded | Values as listed |
...
| chat-page.unit.models.pricing | models.ts | The SHIPPED price table holds its own invariants: every model priced, no cache read above uncached input, every write-billing family (gpt-6, gpt-5.6 and claude) carries `cacheWritePerMillion` at exactly 1.25x input, no other family carries one, the list prices of GPT-6 Sol/Luna, Claude Opus 5.5 and the corrected Claude Opus 4.8 are pinned, and every model has a `maxOutputTokens` between 8000 and its context window | unit | walks MODEL_CONFIGS itself, no fixtures | Iterate every entry | All invariants hold |
...
| chat-page.unit.max-output.004 | models.ts | The shipped ceilings: 32000 for GPT-6, the 5.6 family and 5.5 (reasoning comes out of the same budget), 16000 for Claude and gpt-5.4, 8000 for the small models, and every ceiling inside its context window | unit | walks MODEL_CONFIGS itself | Iterate every entry | Values as listed |
...
| chat-page.unit.reasoning-effort.resolve.006 | reasoning-effort.ts | GPT-6 defaults, xhigh/max overrides and the minimal clamp | unit | same | Resolve gpt-6-sol / gpt-6-luna with no pick; parse `{"claude-opus-5-5":"max","gpt-6-sol":"xhigh","claude-opus-4-8":"xhigh"}`; pick "minimal" on gpt-6-sol and claude-opus-5-5 | low / low; Opus 4.8 entry dropped, the other two kept; "low" |
...
| chat-page.unit.prompt-cache-key.resolve.005 | prompt-cache-key.ts | gpt-5.5 and other families outside PERSONA_CACHE_KEY_FAMILIES keep the thread id (negative) | unit | — | Resolve for 5.5 / 5.4 / claude-sonnet-5 / claude-opus-5-5 / Kimi under persona strategy | `"thread-42"` each |
| chat-page.unit.prompt-cache-key.resolve.006 | prompt-cache-key.ts | GPT-6 gets the persona key like GPT-5.6 | unit | — | Resolve for gpt-6-sol and gpt-6-luna with a persona under persona strategy | `persona:agent-7:<sig>:<shard>` |
...
| api.unit.chat-route.breakpoint.001 | api/chat/route.ts | No breakpoint by default — instructions stay a plain string | unit | flag unset | POST | `typeof options.instructions === "string"` |
| api.unit.chat-route.breakpoint.002 | api/chat/route.ts | Flag on + 5.6 model marks the developer message | unit | `PROMPT_CACHE_PERSONA_BREAKPOINT=true`, promptCacheOptionsSupported | POST | `instructions.providerOptions.openai.promptCacheBreakpoint` is `{mode:"explicit"}` |
| api.unit.chat-route.breakpoint.003 | api/chat/route.ts | Flag on + pre-5.6 model marks nothing (negative) | unit | flag on, gpt-4o | POST | Instructions stay a plain string |
| api.unit.chat-route.breakpoint.004 | api/chat/route.ts | The flag is a no-op on Anthropic: the system-prefix breakpoint is pinned with the flag off AND on | unit | flag unset / `=true`, claude-sonnet-5 | POST twice | `instructions.providerOptions.anthropic.cacheControl` is `{type:"ephemeral"}` both times; no `openai` provider options |
